...
class App(TKMT.ThemedTKinterFrame):
    ...
    def _setup_file_selection(self, parent_frame):
        ...
        ttk.Label(file_frame, text="Input JXR:").grid(row=0, column=0, sticky="e")
        ...
        ttk.Label(file_frame, text="Output JPG:").grid(row=1, column=0, sticky="e")
    ...
    def _setup_parameters(self, parent_frame):
        params_frame = ttk.LabelFrame(parent_frame, text="Parameters")
        params_frame.grid(row=1, column=0, sticky="ew", pady=(0, 10))

        ttk.Label(params_frame, text="Tone Map:").grid(row=0, column=0, sticky="e")
        ...
        ttk.Label(params_frame, text="Pre-gamma:").grid(row=1, column=0, sticky="e")
        self.pregamma_var = tk.StringVar(value=DEFAULT_PREGAMMA)
        ttk.Entry(params_frame, textvariable=self.pregamma_var, width=15).grid(row=1, column=1, padx=5)

        ttk.Label(params_frame, text="Auto-exposure:").grid(row=2, column=0, sticky="e")
        self.autoexposure_var = tk.StringVar(value=DEFAULT_AUTOEXPOSURE)
        ttk.Entry(params_frame, textvariable=self.autoexposure_var, width=15).grid(row=2, column=1, padx=5)

    def _setup_conversion_button(self, parent_frame):
        convert_btn = ttk.Button(parent_frame, text="Convert", command=self.convert_image)
        convert_btn.grid(row=2, column=0, pady=10, sticky="ew")

    def _setup_status_label(self, parent_frame):
        self.status_label = ttk.Label(parent_frame, text="", foreground="blue")
        self.status_label.grid(row=3, column=0, sticky="w")

    def _setup_previews(self, parent_frame):
        previews_frame = ttk.Frame(parent_frame)
        previews_frame.grid(row=0, column=0, sticky="nsew")

        before_frame = ttk.LabelFrame(previews_frame, text="Before Conversion")
        ...
        after_frame = ttk.LabelFrame(previews_frame, text="After Conversion")
    ...
    def browse_input(self):
        ...
        if filename:
            ...
            self.status_label.config(text="Loading preview...", foreground="white")
    ...
    def create_preview_from_jxr(self, jxr_file: str):
        ...
        try:
            ...
            self.status_label.config(text="Preview loaded successfully", foreground="yellow")
    ...
    def convert_image(self):
        ...
        self.status_label.config(text="Converting...", foreground="white")
        self.master.update_idletasks()

        try:
            run_hdrfix(input_file, output_file, tone_map, pre_gamma, auto_exposure)
            self.status_label.config(text="Conversion successful!", foreground="yellow")
